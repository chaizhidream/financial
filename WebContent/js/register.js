...
function checktelephone(){
	...
	var result=telReg.test(telephone);
	if(telephone==""){
  		sp2.innerHTML="手机号不能为空";
  		sp2.style.color="red";
  	}else if(!result){
  		sp2.innerHTML="手机号不正确";
  		sp2.style.color="red";
  	}else{
  		sp2.innerHTML="手机号合法";
...
function checkemail(){
	...
	var emaReg = /^([a-zA-Z0-9_-])+@([a-zA-Z0-9_-])+(.[a-zA-Z0-9_-])+$/;
	var result=emaReg.test(email);
	if(email==""){
  		sp3.innerHTML="邮箱不能为空";
  		sp3.style.color="red";
  	}else if(!result){
...
function myCheck(){
	...
	if(document.getElementById("username").value==""){            
	    ...
	    alert("请输入手机号");          
		document.formregister.telephone.focus();            
		return false;         
	}  
